...
ran_list <- map(vocab_graphs, function(x){
  ...
  POS <- table(factor(POS$POS, levels = c("nouns","verbs","other")))
...
multiSample(ran_list$NDARAL790GMQ_71$POS, vertices_POS_child$v_ID,vertices_POS_child$POS)

balanced_RAN_network(POS_numbers_ASD$NDARAL790GMQ_71, graph_FullNet,vertices_POS_child$POS)
