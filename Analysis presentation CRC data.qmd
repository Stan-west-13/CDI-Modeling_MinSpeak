...
-   All three groups show different network structures across vocabulary
    size, with these trends changing as a function of how those networks
    were constructed.
...
-   The middle group does not have any child with production past 100
    ...
    other groups, so we removed this group in further analyses. We refer
    to our two groups in the subsequent slides as delayed (72 - 110
    months) with lower than normative vocabulary production and
    non-delayed (16 - 47 months) with vocabulary production on-par for
    their age.
...
-   Welch two sample t-test revealed no significant difference in
    productive vocabulary between groups $t(74.54) = -0.42, p = 0.679$.
...
match_noun_assoc <- map(noun_assoc_split, function(x){
  matchit_all <- x %>%
    ...
    matchit(group_two ~ nProduced, data = ., method = "optimal", distance = "glm"  ,ratio = match_ratio)
  match_df <- x %>%
...
-   Keeping both WS and WG forms, we match the no-delay group to the
    $n = 41$ delayed children, allowing the no-delay group to be 2x
    bigger ($n = 82$).
...
-   Noun Features (TAR filtered; interpreted by Chris Cox):
    ...
    -   Median Indegree: There are no significant trends, linear or
        quadratic, nor group effects for indegree.
    ...
    -   ASPL: There are negative linear and positive quadratic trends of
        ASPL for both groups, and this trend does not differ between
        groups, nor is there a difference of means.

-   Noun-only Associations (interpreted by Chris Cox):

    -   Median Indegree: The groups do not differ on indegree; there is
        a negative linear trend (no quadratic), but neither differ
        between groups. There is a positive interaction of the linear
        trend with group, suggesting a sharper decline in indegree for
        the no delay group.

    -   Clustering Coefficient: There are no significant main effects of
        linear or quadratic trend, nor a main effect of group.

    -   ASPL: Both groups have a negative linear trend of ASPL
        (augmented by a positive quadradic trend). There are no group
...
wt <- ifelse(d_splits_all$assoc$`Median Indegree`$group_two == 'delay',match_ratio,1)
