...
-   Noun Features (TAR filtered; interpreted by Chris Cox):
    ...
    -   There is a negative quadratic trend for indegree, and no linear
        trend, for both groups. There is a main effect of group whereby
        indegree is higher on average for the delayed group.

    -   There are no trends, linear or quadratic, for the clustering
        coefficient, but there is a main effect of group whereby the
        delayed group tends to have higher clustering coefficients on
        average.

    -   There is a negative linear trend (and no quadratic trend) of
...
-   Noun-only Associations (interpreted by Chris Cox):
    ...
    -   The groups do not differ on indegree; there is a negative linear
        and a positive quadradic trend, but neither differ between
        groups, nor is there a difference in the group means.

    -   Both groups have negative linear and quadradic trends in their
        clustering coefficients as vocabularies grow, but the "delayed"
        group has a steeper negative linear trend than the "typical"
        group. However, the group means do not differ.

    -   Both groups have a negative linear trend of ASPL (augmented by a
        positive quadradic trend), but there is a main effect such that
        paths are longer in the delayed group on average. There is
        almost an interaction on the linear trend, which would have
        suggested that ASPL declines more slowly for the delayed group,
        but this interaction is not actually significant (p = .0596).
...
wt <- ifelse(d_splits_all$assoc$`Median Indegree`$group_two == 'delay',3,1)
