...
-   We allowed WG to be as big as the ASD group and WS to be 3x bigger in the TD group. These were the best ratios for optimized matching. Allowing more kids in the WG group exacerbated differences in vocabulary especially earlier and later in noun production. 
...
-   3-way ANOVA revealed no differences in number of nouns produced
    between the TD and two ASD groups $F(2,340) = 0.73, p = 0.481$.
...
save(match_noun_assoc,file = "data/matched_ASD_df.Rdata")
...
## Combine df for BS in OSG

df_osg <- map_dfr(matched_df_poly_all, function(x){
  as.data.frame(x)
  })

write_rds(df_osg, file = "data/asd_TD_minspeak_osg.rds")
