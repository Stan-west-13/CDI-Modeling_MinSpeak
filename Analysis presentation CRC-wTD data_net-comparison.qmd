...
d_matched_td <- read_rds("data/matched-td_2026-03-21.rds")
...
## Typically Developing Total Word Production
-   We allowed WG to be 2x bigger and WS to be 3x bigger in the TD group. These were the best ratios for optimized matching.

```{r,message=FALSE, echo =  FALSE}
d_matched_td %>%
  select(subjectkey,form, nproduced_total,nouns_produced = nproduced, interview_age) %>%
    pivot_longer(cols = c("nouns_produced", "nproduced_total"),
                 names_to = "measure",
                 values_to = "nproduced")%>%
  group_by(measure,form) %>%
  get_summary_stats(nproduced, type = c('full')) %>%
  as.data.frame()
...
ggplot(d_matched_td%>%
  select(subjectkey, form,nproduced_total,nouns_produced = nproduced, interview_age) %>%
    pivot_longer(cols = c("nouns_produced", "nproduced_total"),
                 names_to = "measure",
                 values_to = "nproduced"), aes(x = nproduced, y = interview_age))+
  geom_point(size = 6)+
  facet_grid(form~measure,scales = "free") +
...
-   3-way ANOVA revealed no differences in number of nouns produced
    between the TD and two ASD groups $F(2,413) = 0.20, p = 0.819$.

```{r}


match_noun_assoc <- d_netstats_first %>%
    select(subjectkey, interview_age,nProduced, group_two) %>%
    unique() %>%
    filter(nProduced <= max(nProduced[group_two == "delay"]+20)) %>%
    matchit(group_two ~ nProduced, data = ., method = "optimal", distance = "glm"  ,ratio = 2)

match_noun_assoc <- d_netstats_first %>%
    filter(subjectkey %in% match.data(match_noun_assoc)$subjectkey) 
  
save(match_noun_assoc,file = "data/matched_ASD_df-netcomp.Rdata")
...
combined <- rbind(match_noun_assoc, d_netstats_TD)

d_aov <- combined %>%
    select(subjectkey,nProduced,group_two) %>%
    unique()  
summary(aov(nProduced~group_two, data = d_aov))
...
```{r, echo =  FALSE,fig.width=48, fig.height=12, fig.fullwidth=TRUE}
  ...
  combined %>%
    select(subjectkey,form,interview_age,nProduced,group_two) %>%
    unique() %>%
    group_by(group_two,form) %>%
    summarize(n = n(),
              mean_age = mean(interview_age),
...
ggplot(combined, aes(x = nProduced, color = group_two))+
  geom_density(size = 2)+
  theme_bw(base_size = 52)+
  labs(title = "Matching between groups")


ggplot(combined, aes(x = nProduced, , y = interview_age,color = group_two))+
  geom_point(size = 13)+
  theme_bw(base_size = 52)+
  labs(title = "Matching between groups ")
...
-   Noun Features (TAR filtered; interpreted by Chris Cox):
    ...
    -   Median indegree: There is a significant linear trend in median
        indegree, as well as significant mean differences between the TD
        and both the delay and no-delay groups. A signficant negative
        interaction between group and linear trends indicate that the TD
        group is increasing faster than both delay and no-delay groups.

    -   Clustering Coefficient: There are no trends, linear or
        quadratic, for the clustering coefficient, but there is a main
        effect of group whereby the delayed group tends to have higher
        clustering coefficients on average.

    -   ASPL: There is a negative linear trend (and no quadratic trend),
        as well as mean differences between both ASD groups (delay and
        no-delay) and the TD group. There is also a significant linear
        by group interaction for both groups indicating that the TD
        group is achieving low path lengths faster. Finally, there is a
        significant quadratic by group interaction for the no delay
        group, indicated by a late increase in path length around 200
        nouns.

-   Noun-only Associations (interpreted by Chris Cox):

    -   Median Indegree: There is a positive linear and a negative
        quadradic trend, as well as a significant negaitive group
        difference between both ASD groups and the TD group. There are
        negative linear and positive quadratic by no-delay group
        interactions, captured by rapid decreases in indegree early on
        for the no-delay group, whereas the TD group has steady
        increases over noun acquisition.

    -   Clustering Coefficient: There are positive linear and negative
        quadratic trends as well as significant group differences
        between the TD and both ASD groups. There are also significant
        negative linear and positive quadratic by no-delay group
        interactions, suggesting steeper decrease in clustering for the
        no-delay group, with steady increases that plateau in the TD
        group. There was finally a significant negaitve linear by delay
        group interaction, again with this group decreasing faster than
        the TD group.

    -   ASPL: There are negative linear and positive quadratic trends in
        ASPL, with no significant group differences. There are also
        positive linear by delay/no-delay interactions, suggesting
        quicker decrease in ASPL for the TD group early in noun
        acquisition. There was also a negative quadratic by delay group
        interaction, suggesting that the TD group has quicker increase
        in ASPL later in noun learning.

```{r, echo = TRUE}

nProduced_poly_all <- combined %>%
        ungroup() %>%
        select(nProduced) %>%
        unique() %>%
...
matched_df_poly_all <- combined %>%
                          left_join(nProduced_poly_all) %>%
                          filter(type == "vocab")

d_splits_all <- split(matched_df_poly_all,matched_df_poly_all$netstat)

 map(d_splits_all, function(y){
...
wt <- ifelse(d_splits_all$dist$group_two == 'delay',2,1)

x<- map(d_splits_all, function(d){ 
      d$group_two <- relevel(d$group_two, ref = "TD")
          lm(z ~ (linear+quadratic)*group_two*network, data = d,weights = wt)
      })


map(x,function(d){summary(d)})
...
```{r}
d_plot_pred <- readRDS("data/combined_predictions.RDS")
ggplot(d_plot_pred %>% 
  filter(type == "vocab"), aes(x = nProduced, y = z, color = group_two,linetype = network))+
  #geom_point(alpha = 0.2)+
  theme_bw(base_size = 24) +
  geom_line(aes(x = nProduced, y = prediction))+
  facet_grid(form~metric_plot)+
