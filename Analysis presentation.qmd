...
ggplot(d_netstats %>% 
  ...
  facet_grid2(~metric_plot,scales = "free_y", independent = "y")+
...
-   Linear models were run regressing
    $network_{statistic} \sim (linear_{nproduced} + quadratic_{nproduced} +cubic_{nproduced}) * age * group$
...
x <- map(d_splits_all, function(x){
  lm(z ~ (linear+quadratic+cubic)*interview_age*group_two, data = x)
})
